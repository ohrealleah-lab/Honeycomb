...
struct SpiderTouchView: View {
    ...
    var body: some View {
        GeometryReader { geo in
            ...
            // ZStack siblings, that would make the ZStack itself adopt the full-bleed,
            // notch-including size and propose it to every other sibling too, shifting
            // the whole board out past the safe area right as the win/stuck banner
    ...
    // Matches mac's SpiderView.requestNewGame() — a no-progress board (nothing dealt
    // yet, or dealt but untouched) starts over silently; anything past that confirms
    // first, since it's about to discard real progress.
    ...
    private func tableauColumn(pile: Pile, cardW: CGFloat, cardH: CGFloat) -> some View {
        ...
        // branch (SpiderViews.swift) that highlights the destination's top card.
        let hintIndex: Int? = {
