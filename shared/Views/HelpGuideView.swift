    ...
    var body: some View {
        HelpShell(title: "Honeycomb", subtitle: "A tactical 3×3 grid card battle inspired by Triple Triad.") {
            ...
            RuleSection(title: "🎲 Match Modifiers & Special Rules",
                        text: "• Symmetry: If 2+ touching neighbor stats match your card's facing stats, all matching neighbors are captured simultaneously.\n• Nectar Math: If the sum of (your stat + neighbor's stat) equals the same total across 2+ neighbors, all involved cards are captured.\n• Queen's Fall: A card with a stat of 1 attacking a 10 (\"A\") always captures it!\n• Inversion: Inverts all comparisons—lower stats beat higher stats. (Inversion appears via Roulette only).\n• Pollination / Smoked Out: Grants +1 (or −1) to stats for all cards matching randomly selected suits as more of that suit enter the board.\n• Clear Skies: Both players' hands are completely visible.\n• Scouting Party: Three random cards in each player's hand are visible.\n• Nectar Exchange: One random card is swapped between the players' hands at the start of the match.\n• Hierarchy / Frenzy: Hierarchy forces you to play cards in exact deck sequence. Frenzy randomly mandates which card must be played each turn.\n• Hive Swarm: First card played remains face-down for 3 turns before flipping automatically.\n• Swarm to the Death: If the match ends in a draw, a rematch is immediately played with the cards currently owned on the board.")
