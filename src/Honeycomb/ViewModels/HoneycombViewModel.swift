...
public final class HoneycombViewModel {
    ...
    public func aiPlayTurn() {
        ...
        let empties = Self.emptyBoardIndices(board: boardSnapshot)

        func computeMove() -> (handIndex: Int, boardIndex: Int)? {
            switch difficulty {
            case .easy:
                return Self.randomMove(eligibleHands: eligibleHands, empties: empties)
            case .medium:
                return Self.greedyMove(board: boardSnapshot, opponentDeck: opponentDeckData, eligibleHands: eligibleHands, empties: empties, rules: rules)
            case .hard:
                return Self.minimaxMove(board: boardSnapshot, opponentDeck: opponentDeckData, playerDeck: playerDeckData, eligibleHands: eligibleHands, empties: empties, rules: rules, lookaheadPlies: 2)
            case .ultraHard:
                return Self.minimaxMove(board: boardSnapshot, opponentDeck: opponentDeckData, playerDeck: playerDeckData, eligibleHands: eligibleHands, empties: empties, rules: rules, lookaheadPlies: 6)
            }
        }
    ...
    //
    // Everything below is `static` and takes its board/hand/rules state as plain value-
    // type parameters (no reference to `self`) so it's safe to run on a background queue.

    private static func emptyBoardIndices(board: HoneycombBoard) -> [Int] {
    ...
    private static func randomMove(eligibleHands: [Int], empties: [Int]) -> (handIndex: Int, boardIndex: Int)? {
    ...
    private static func greedyMove(board: HoneycombBoard, opponentDeck: [HoneycombCardData], eligibleHands: [Int], empties: [Int], rules: [HoneycombRule]) -> (handIndex: Int, boardIndex: Int)? {
        guard !empties.isEmpty, !eligibleHands.isEmpty else { return nil }
    ...
    private static func minimaxMove(board: HoneycombBoard, opponentDeck: [HoneycombCardData], playerDeck: [HoneycombCardData], eligibleHands: [Int], empties: [Int], rules: [HoneycombRule], lookaheadPlies: Int) -> (handIndex: Int, boardIndex: Int)? {
        guard !empties.isEmpty, !eligibleHands.isEmpty else { return nil }
    ...
    private static func minimaxScore(
        board: HoneycombBoard,
    ...
    private static func positionalWeight(_ index: Int) -> Int {
        switch index {
    ...
    private static func positionalEvaluation(board: HoneycombBoard) -> Int {
        var score = 0
