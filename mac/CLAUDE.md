...
3. For a per-game field: add `@State` + init seed + an `updatedOpts` assignment in that game's Options view, as usual. For a `SharedGameOptions` field with **no non-idempotent side effect on commit** (the common case — matches Sound/Honey Mode/Manually Dismiss Banners/Hide Hint Button): don't add local `@State` at all — bind its Toggle straight to `$coordinator.<field>` in the sheet's `content`, and add capture-in-`init`/revert-in-Cancel lines to `OptionsSheetShell.swift` itself (next to `originalIsSoundEnabled` and friends) so every sheet gets Cancel-to-discard for free, matching how the Themes sub-panel already works. For a `SharedGameOptions` field that **does** have a non-idempotent side effect when it changes (the one existing example: `noStressMode` triggers "deal a fresh hand" in Video Poker/Blackjack/Honeycomb if toggled on mid-hand) — live-binding it would fire that side effect on every toggle flip during preview, including ones the player then cancels, which can't be undone. Keep that field locally buffered the old way (`@State` + init seed from `coordinator.<field>` + `coordinator.<field> = <field>` in `onOK`) instead of adding it to the shell.
