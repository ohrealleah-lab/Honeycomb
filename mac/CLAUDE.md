...
The single source of truth for which game is active. Holds all six ViewModels (`klondikeViewModel`, `beecellViewModel`, `spiderViewModel`, `videoPokerViewModel`, `blackjackViewModel`, `honeycombViewModel`) alive simultaneously. On `gameMode.didSet` it calls `syncSharedOptions(from:to:)`, which reads `commonOptions` (see `HasCommonGameOptions` below) off the outgoing game and assigns it to every other game, so shared preferences stay in sync across mode switches without a per-field switch statement.

### Per-game structure (repeated for Klondike / Beecell / Spider)
Each game follows the same three-layer pattern:
- **Model**: `GameOptions` / `BeecellOptions` / `SpiderOptions` — `Codable` structs persisted as JSON to `UserDefaults` (`"solitaire_options"`, `"beecell_options"`, `"spider_options"`). Always use `decodeIfPresent ?? default` in `init(from:)` so new fields survive old saves without migration. Theme fields (felt color, card back, custom card colors, felt vignette, custom background) do **not** live here — see "App-wide theme" below. Every game's Options struct (including `VideoPokerOptions`, `BlackjackOptions`, and `HoneycombViewModel.Options`) conforms to `HasCommonGameOptions` (`shared/Models/CommonGameOptions.swift`), exposing the fields that are conceptually shared across games (`isSoundEnabled`, `noStressMode`, and the Optional `hideHintButton`/`showPointHighlights`/`isTimed` for games that have those concepts) as a single `commonOptions` computed property — this is what `AppCoordinator.syncSharedOptions` reads/writes generically instead of switching on each field.
