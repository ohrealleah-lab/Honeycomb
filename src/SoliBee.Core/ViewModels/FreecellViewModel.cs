...
public partial class FreecellViewModel : ObservableObject
{
    ...
    // Dry-run the same greedy strategy Autocomplete() uses (repeatedly send any
    // playable tableau/free-cell top card to a foundation) against copies of the
    // pile contents, without mutating real state. This is the correct test for
    // "can the board be auto-finished from here" — requiring the tableau to
    // already be a perfectly sorted run (the old check) rejects plenty of
    // legitimately auto-finishable positions, e.g. interleaved sorted columns.
    private bool SimulateAutocomplete()
    {
        ...
        var foundationTops  = Foundations.Select(f => f.Cards.Count == 0 ? null : f.Cards[^1]).ToList();
        ...
        while (movedAny)
        {
            ...
            foreach (var tab in tableauCopies)
            {
                ...
                var card = tab[^1];
                int idx = FindFoundationSlot(foundationTops, card);
                if (idx < 0) continue;
                foundationTops[idx] = card;
    ...
    private static int FindFoundationSlot(List<Card?> foundationTops, Card card)
    {
        for (int i = 0; i < foundationTops.Count; i++)
        {
            var top = foundationTops[i];
            if (top == null && card.Rank == 1) return i;
            if (top != null && top.Suit == card.Suit && card.Rank == top.Rank + 1) return i;
        }
        return -1;
    }
    ...
    public void Autocomplete()
    {
        ...
        while (_foundationCardCount < ExpectedCards)
        {
            ...
            if (!moved)
            {
                foreach (var cell in FreeCells.Where(c => c.Cards.Count > 0))
                {
                    ...
                    foreach (var foundation in Foundations)
                    {
                        if (CanMoveCards(new List<Card> { card }, foundation))
                        {
                            MoveCards(new List<Card> { card }, cell, foundation);
                            moved = true;
                            break;
                        }
                    }
