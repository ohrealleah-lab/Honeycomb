...
The player can request a move hint at any time during Klondike, BeeCell, or Spider games. A set of keyboard shortcuts provides quick access to common actions without using the mouse. The game board supports keyboard cursor navigation for accessible play. (Honeycomb has its own, separately-specified hint system — see User Story 11, Acceptance Scenario 14 — since it suggests an AI-evaluated move rather than a pile-to-pile card move.)
...
11. **Given** the player wins a match, **When** the post-game prompt appears, **Then** the player may drag one card the opponent played that round onto one of their own 5 active deck slots to swap it in, with a confirmation prompt before the swap completes; this steal is capped at one per match — once spent, the Steal Card option is hidden until the player Rematches (or starts a new match), even if multiple opponent cards were captured that round.
...
14. **Given** it is the player's turn and the match difficulty is not Ultra Hard, **When** the player presses the Hint button, **Then** the mathematically best card-and-cell placement (found by the same minimax search Ultra Hard's AI uses) is highlighted on the suggested hand card and board cell for 2 seconds before auto-clearing; the Hint button itself is not shown at all on Ultra Hard difficulty, and the search only ever considers opponent cards actually revealed to the player (e.g. via All Open/Three Open), never the opponent's hidden hand.
...
- **Honeycomb Hint Under Order/Chaos**: If Order or Chaos restricts the player to exactly one legal hand card this turn, the Hint system only searches for the best board cell for that mandated card, not which card to play.
- **Honeycomb Hint Staleness**: If the board changes (the player plays a card by hand, undoes a move, or the match ends) while a Hint's background search is still computing, the in-flight result is discarded rather than applied to a board it no longer matches.
...
- **FR-043**: Up to two rules MUST be selectable simultaneously from the pool {Ascension, Descension, Same, Plus, Fallen Ace, Reverse, All Open, Three Open, Swap, Order, Chaos, Bomb Shelter}; Ascension/Descension MUST be mutually exclusive, and Order/Chaos MUST be mutually exclusive.
...
- **FR-049**: Order MUST restrict the legal play each turn to the next card in that side's original deck order; Chaos MUST re-roll a single random legal card index for that side the instant its turn begins. The currently-mandated card MUST be highlighted with a thick yellow border, and the opponent's AI MUST wait at least 2 seconds (with the mandated card visibly highlighted) before executing its move.
- **FR-049b**: Bomb Shelter MUST keep the first card a player plays face down, unknown to the opponent. At the end of the game, the face down cards are flipped (starting player’s card exposed first). Face down cards MUST NOT trigger combos upon placement.
- **FR-050**: AI difficulty MUST scale by algorithm: Easy = uniform random move; Medium = greedy (maximizes this move's own capture count, ties broken randomly); Hard = minimax with alpha-beta pruning, 2-ply lookahead; Ultra Hard = minimax with alpha-beta pruning, 6-ply lookahead. Both minimax tiers score non-terminal boards with a positional heuristic (each occupied cell scored by ownership plus how exposed its real, modifier-adjusted stats are toward each currently-empty neighboring cell, plus a Same/Plus combo-potential term when those rules are active); Ultra Hard's heuristic additionally docks cells exposing the one stat value Fallen Ace could exploit (an Ace, or a `10` under Reverse) — Hard's shallower search omits that extra caution layer.
...
- **FR-052**: Under Reverse, each difficulty MUST use a dedicated Reverse-specific deck composition instead of its normal one, so that a nominally "hard" AI cannot be trivially defeated by exploiting the low-beats-high inversion: Easy borrows Hard's normal table; Medium reuses its own normal table (already low-stat-heavy enough); Hard uses an explicit two 1★ + three 2★ table; Ultra Hard uses five 1★ cards.
...
- **FR-055**: A fresh install MUST grant three randomly-selected 1★ cards and two randomly-selected 2★ cards to the Card Bank as starter unlocks. The Start Over action MUST redraw Deck 1 with fresh cards matching its previous star-tier composition (falling back to the default 1★/2★ mix if empty), renaming it "Default" and making it the active deck.
- **FR-056**: Winning a match MUST NOT, by itself, unlock any card to the Card Bank — the player's own deck cards are already unlocked (they came from the Card Bank to begin with), and no other card is granted automatically. The Card Bank's only growth path is the explicit post-win "Take a Card" swap (FR-057).
- **FR-057**: After a won match, the player MUST be able to drag one card the opponent originally played that round — provided the player actually captured it (owns it at match end) and doesn't already have it unlocked — onto one of their own 5 active deck slots to swap it in, subject to a confirmation prompt; this both unlocks the taken card to the Card Bank and replaces the outgoing slot's card in the active deck (the outgoing card remains unlocked in the Card Bank/Saved Decks, it's just no longer in this deck slot). This steal MUST be capped at one per match, even if the player captured multiple opponent cards that round — the Steal Card option MUST be hidden once spent, until the player Rematches or starts a new match.
- **FR-058**: The player profile MUST support 5 named saved-deck slots. Saved deck names MUST be editable at any time after being saved, with names capped at 20 characters. The starter deck MUST be named "Default" and set as active from the start.
- **FR-059**: The card database MUST contain 552 cards split evenly across 4 suits (138 each), with per-tier stat ceilings of 7/7/8/9/10 for 1★-5★ (a `10` value MUST be exclusive to 5★ cards) and per-tier total-stat-sum bands of 12-15/16-21/20-25/24-28/25-30.
...
- **FR-061**: Statistics tracked for Honeycomb MUST include: games played/won/lost/drawn, current and longest win streak, win percentage, wins broken down by difficulty (Easy/Medium/Hard/Ultra Hard), total unique Card Bank unlocks (of 552) with a per-suit breakdown (of 138 each) and per-star-rating breakdown, flawless (10-0) victory count, lifetime cards flipped, lifetime Same/Plus trigger count, lifetime Fallen Ace captures (a `1` successfully capturing a `10`, regardless of which rule authorized the capture), lifetime cards stolen via the post-win "Take a Card" swap, and the number of times a match has entered Sudden Death overtime.
- **FR-062**: A Hint action MUST be available during the player's turn on any difficulty below Ultra Hard (hidden entirely on Ultra Hard), suggesting the single hand-card-and-board-cell placement found by the same Ultra-Hard-caliber minimax search regardless of the match's own difficulty; the suggestion MUST only ever be computed from opponent cards actually revealed to the player, never their hidden hand. The suggested card and cell MUST be highlighted for 2 seconds and then auto-clear, one suggestion per press, matching the existing hint presentation used elsewhere in the app.
...
- **HoneycombRule**: one of Ascension, Descension, Same, Plus, Fallen Ace, Reverse, All Open, Three Open, Swap, Order, Chaos, Bomb Shelter.
...
- **HoneycombStats**: games played/won/lost/drawn, streaks, wins by difficulty (Easy/Medium/Hard/Ultra Hard), flawless victories, lifetime cards flipped, lifetime Same/Plus triggers, lifetime Fallen Ace captures, lifetime cards stolen, Sudden Death entry count.
...
- **SC-004**: The Klondike/BeeCell/Spider hint engine returns a result in under 100ms. (Honeycomb's Hint is a separate, much heavier search — see SC-011 — and is computed off the main thread specifically because it doesn't meet this bar.)
...
- **SC-011**: Honeycomb's Ultra Hard AI (6-ply minimax with alpha-beta pruning) selects a move within a few seconds in the worst case (full hands, empty board — up to ~2.6s observed); this is always computed off the main thread so the UI never freezes. Hint (which reuses this same search regardless of the match's own difficulty) has the identical worst case and the identical off-main-thread guarantee.
...
- **Honeycomb Card Ownership Colors**: Mandated-card (Order/Chaos) highlighting uses a fixed thick yellow border; there is no user-configurable per-side highlight color.
